...
var _primed_jump := false
var _duck_jump := false
...
func start(entity: Node2D) -> Variant:
	...
	# 满速奔跑时使用蓄力跳贴图
	_primed_jump = abs(player.velocity.x) >= player.max_run_speed
	
	# 下蹲时跳跃保持下蹲贴图
	_duck_jump = Input.is_action_pressed("player_duck")
