...
## Refreshes the icon based on the current theme.
func refresh_icon() -> void:
	if part != null and is_node_ready():
		var env = _editor().level.current_sub_area
		if env != null:
			%Icon.texture = part.get_part_icon(env)
			%Icon.texture_filter = part.get_part_icon_filter(env)
