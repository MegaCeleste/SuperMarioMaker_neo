...
var _move_dir := 1.0  # Last movement direction (1=right, -1=left)
...
func _ready() -> void:
	...
	# Check if player already overlaps at spawn (body_entered won't fire then)
	call_deferred(&"_check_initial_overlap")


func _check_initial_overlap() -> void:
	for body in $KillArea.get_overlapping_bodies():
		if body.get_script() == load("res://scripts/level/entities/player.gd"):
...
func _physics_process(delta: float) -> void:
	match _state:
		...
		State.WALKING:
			move_and_slide()
			# Check if hitting a shell to re-enter it (before SlideComponent turns around)
			if is_on_wall():
				for i in get_slide_collision_count():
					...
					if shell != null:
						...
						if pickup == null or not pickup.held:
							...
							return
			# Also check ShellSensor for non-colliding overlaps
		...
		State.SQUISHED:
			...
			if _squish_timer <= 0:
				queue_free()
		State.REENTERING:
			# Animation handled by tween, no physics processing needed
			pass
...
func _reenter_shell(shell: Shell) -> void:
	# Beach koopa jumps back into an empty shell
	_state = State.REENTERING
	$SlideComponent.process_mode = Node.PROCESS_MODE_DISABLED
	var move_dir = sign(velocity.x) if absf(velocity.x) > 0.5 else 1.0
	velocity = Vector2.ZERO
	...
	tween.tween_callback(func():
		...
		shake.tween_callback(func():
			var koopa = load("res://scenes/entities/koopa.tscn").instantiate()
