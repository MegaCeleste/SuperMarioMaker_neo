...
@onready var _tex: AtlasTexture = %Sprite.texture as AtlasTexture


static func get_part_icon(environment: SubArea) -> Texture2D:
	match environment.level_theme:
		Level.LevelTheme.AIRSHIP:
			return preload("res://textures/ui/editor/part_icons/smw/ground_airship.png")
	return preload("res://textures/ui/editor/part_icons/smw/ground.png")


static func _theme_ground_atlas(theme: int) -> Texture2D:
	match theme:
		Level.LevelTheme.AIRSHIP:
			return load("res://textures/tiles/ground_airship.png")
	return load("res://textures/tiles/ground.png")


func _update_theme_texture() -> void:
	if sub_area == null:
		return
	_tex.atlas = _theme_ground_atlas(sub_area.level_theme)
	refresh_sprite()


# Only updates the atlas texture without re-running auto-tile.
func _set_theme_atlas(theme: int) -> void:
	_tex.atlas = _theme_ground_atlas(theme)
...
func _get_nearby_tiles(
	...
	for x in range(-2, 3):
		for y in range(-1, 2):
			...
			query.collide_with_areas = true
			query.collide_with_bodies = false
			query.collision_mask = 1 << 8  # Layer 9 for real GroundParts
			query.exclude = [get_rid()]
			...
			if not result.is_empty():
				...
				if coll is GroundPart:
					base[Vector2i(x, y)] = coll
					continue
			# Fallback: check editor_preview group (StartingGround preview tiles)
			var found: GroundPart = null
			var target_grid = pos + Vector2i(x, y)
			for p in get_tree().get_nodes_in_group(&"editor_preview"):
				if p.is_queued_for_deletion():
					continue
				if p is GroundPart and Level.to_grid(p.global_position) == target_grid:
					found = p
					break
			base[Vector2i(x, y)] = found
	return base
