...
func start(entity: Node2D) -> Variant:
	...
	# Disconnect any existing _fall connection to prevent duplicate
	for c in _death_timer.timeout.get_connections():
		if c.callable.get_method() == "_fall":
			_death_timer.timeout.disconnect(c.callable)
