...
## Returns variant part scenes for the variant selection window.
## Override in subclasses to provide variants.
...
func _create_window() -> void:
	...
	_window.selected.connect(_on_variant_selected)
	for variant_scene in variants:
		var part_script = variant_scene.instantiate().get_script()
		var icon = null
		if part_script.has_method("get_part_icon"):
			icon = part_script.get_part_icon(sub_area)
		...
		var scene_path = variant_scene.resource_path
		btn.pressed.connect(func():
			_window.select(scene_path)
		)
		_window.get_node("VBox").add_child(btn)
	level.editor.get_node(^"%WindowLayer").add_child(_window)


func _on_variant_selected(id: StringName) -> void:
	var scene = load(id) as PackedScene
	if scene == null:
		return
	var new_part = scene.instantiate() as Part
	if new_part == null:
		return
	# Replace current part with the variant at the same position
	new_part.global_position = global_position
	new_part._grid_pos = _grid_pos
	new_part.sub_area = sub_area
	new_part.level = level
