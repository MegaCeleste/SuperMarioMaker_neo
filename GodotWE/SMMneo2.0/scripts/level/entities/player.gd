...
var direction := 1:
	set(value):
		if value != direction and _held_item != null:
			...
			# Flip held item sprite to match facing direction
			var held_sprite = _held_item.get_node_or_null("Graphics/Sprite")
			if held_sprite:
				held_sprite.flip_h = value < 0
			# Raise shell above player during the pass-through animation
			var held_sprite = _held_item.get_node_or_null("Graphics/Sprite")
			if held_sprite:
				held_sprite.flip_h = value > 0
			_held_item.z_index = 10
			held_item_tween = create_tween().set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_SINE)
			held_item_tween.tween_property(self._held_item, "position", Vector2(0, -1.5), 0.01)
			held_item_tween.tween_interval(7/60.0)
			held_item_tween.tween_property(self._held_item, "position", Vector2(-11 * direction, -1.5), 0.05)
			held_item_tween.tween_callback(func():
				if _held_item != null:
					_held_item.z_index = -1
			)
			direction = value
...
## Timer that prevents state machine from overriding the kick animation.
var kick_anim_timer := 0.0
var _p_timer := 0.0
...
func _ready() -> void:
	...
	# level.hud.player = self 会导致无法进入游戏
	# trigger setter
...
func _physics_process(delta: float) -> void:
	super(delta)
	if kick_anim_timer > 0:
		kick_anim_timer -= delta
	if _held_item != null:
		# Sync held item sprite direction every frame
		var held_sprite = _held_item.get_node_or_null("Graphics/Sprite")
		if held_sprite:
			held_sprite.flip_h = direction > 0
		if not Input.is_action_pressed("player_run"):
			drop_item()
	_attempt_correction(delta, 2)
	move_and_slide()
	# Detect kicking a stationary shell by walking into it
	if kick_anim_timer <= 0:
		for i in get_slide_collision_count():
			var coll = get_slide_collision(i)
			if coll.get_collider() is Shell or (coll.get_collider() is CharacterBody2D and coll.get_collider().get_script() == Shell):
				sprite.play("kick")
				kick_anim_timer = 0.15
				sounds.stream = preload("uid://c345xnns7om3m")
				sounds.play()
				break
...
func drop_item() -> Entity:
	...
	_held_item.z_as_relative = false
	sprite.play("kick")
	kick_anim_timer = 0.15
	sounds.stream = preload("uid://c345xnns7om3m")
	sounds.play()
	var coll = KinematicCollision2D.new()
	if _held_item.is_inside_tree() and _held_item.test_move(global_transform,
...
func _deferred_hold(item: Entity, pickup_comp: PickupComponent) -> void:
	item.reparent(self)
	# Always place shell in front of Mario based on his facing direction
	item.position = HELD_ITEM_OFFSET * Vector2(direction, 1)
	# Flip held item sprite to match facing direction
	var held_sprite = item.get_node_or_null("Graphics/Sprite")
	if held_sprite:
		held_sprite.flip_h = direction > 0
	_held_item = item
